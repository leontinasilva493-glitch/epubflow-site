...
export default async function middleware(req: NextRequest) {
  ...
  // Check for session using Better Auth's official getSessionCookie utility
  ...
  // Hide template marketing pages for static v1 launch.
  const blockedMarketingPrefixes = [
    '/ai',
    '/blog',
    '/docs',
    '/about',
  ...
  if (
    blockedMarketingPrefixes.some(
      (prefix) =>
        pathnameWithoutLocale === prefix ||
        pathnameWithoutLocale.startsWith(`${prefix}/`)
    )
  ) {
    const locale = LOCALES.find((l) => nextUrl.pathname.startsWith(`/${l}/`));
